      ...
      <div class="checkout-tit">
        ...
        <div class="paymark">
          <span class="fl">
            ...
            <em>{{ this.$route.query.orderId }}</em>
          </span>
          <span class="fr">
          ...
          beforeClose:(action, instance, done)=>{
            switch (action) {
              case 'confirm':
                ...
                break
              case 'cancel':
                //用户点击了取消或者遇到支付问题，清楚定时器，弹窗提示
                alert('出现支付问题请联系QQ:490660221')
                window.clearInterval(this.timer)
                this.timer = null
                done()
                break
    ...
    //开启定时器不断询问服务器支付情况
     if (!this.timer) {
       this.timer = window.setInterval(async () => {
         const result = await this.$API.reqPayStatusApi(this.payInfo.orderId)
         if (result.code === 200) {
           //用户已经支付成功，保存成功码，清楚定时器，路由跳转
